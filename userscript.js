...
// @version      1.0.1
// @description  Activity status, travel icons, @mention alerts, search system, real timestamps – by sercann
// @author       sercann
...
(function () {
    ...
    function saveCfg(c) { GM_setValue(STORE_KEY, JSON.stringify(c)); }
    ...
    ══════════════════════════════════════════ */
    GM_addStyle(`
        .bfc-dot { display:inline-block; width:7px; height:7px; border-radius:50%; margin:0 4px 0 1px; vertical-align:middle; flex-shrink:0; transition:background .35s,box-shadow .35s; }
        .bfc-state-icon { font-size:11px; margin:0 2px 0 0; vertical-align:middle; cursor:default; line-height:1; }
        ...
        .bfc-tb-btn { background:#2d2d2d; border:1px solid #444; color:#ccc; border-radius:4px; font-size:11px; padding:3px 6px; cursor:pointer; transition:all .15s; white-space:nowrap; line-height:1.4; }
        ...
        .bfc-s-active { background:rgba(41,182,246,.3)!important; outline:1px solid ${CFG.accentColor}; border-radius:2px; }
        #bfc-search-count { font-size:11px; color:#777; min-width:52px; text-align:center; white-space:nowrap; }
        ...
        #bfc-member-popup { position:fixed; z-index:9999999; background:#0d1117; border:1px solid #21262d; border-radius:10px; box-shadow:0 8px 28px rgba(0,0,0,.7); padding:8px 0; width:250px; max-height:340px; overflow-y:auto; display:none; font-family:'Segoe UI',sans-serif; font-size:12px; }
        ...
        .bfc-mp-item { display:flex; align-items:center; padding:5px 12px; gap:6px; transition:background .1s; }
        ...
        .bfc-mp-state { font-size:12px; flex-shrink:0; }
        .bfc-col-on   { color:${CFG.statusColors.online}; }
        .bfc-col-idle { color:${CFG.statusColors.idle}; }
        .bfc-col-off  { color:#484f58; }
        ...
        #bfc-gear-btn:hover { background:${CFG.accentColor}; border-color:${CFG.accentColor}; color:#fff; transform:rotate(60deg) scale(1.08); }
        ...
        .bfc-vol-row { display:flex; align-items:center; gap:8px; margin-bottom:12px; }
        .bfc-vol-row span.lbl { color:#8b949e; font-size:12px; flex:1; }
        .bfc-vol-row span.val { color:${CFG.accentColor}; font-size:11px; min-width:34px; text-align:right; font-weight:600; }
        input[type=range].bfc-rng { flex:2; accent-color:${CFG.accentColor}; }
    ...
    async function pollStatuses() {
        ...
        Object.entries(d.members).forEach(([id, m]) => {
            ...
            memberCache[id] = { name: m.name, status, lastSeen: m.last_action ? m.last_action.timestamp : 0, state: rawState, stateDesc };
    ...
    function refreshDot(dot, info) {
        if (!info) { dot.style.background = CFG.statusColors.offline; dot.style.boxShadow = 'none'; dot.title = 'Unknown'; return; }
        const color = dotColor(info.status);
        dot.style.background = color;
        dot.style.boxShadow  = info.status !== 'offline' ? `0 0 5px ${color}` : 'none';
        dot.title = capitalize(info.status) + (info.lastSeen ? ' · ' + timeAgo(info.lastSeen) : '');
    }
    function refreshStateIcon(el, info) {
        if (!info || !CFG.showTravelIcons) { el.textContent = ''; return; }
        const icon = STATE_ICONS[info.state] || '';
        el.textContent = icon;
        if (icon) el.title = info.stateDesc || capitalize(info.state);
    }
    function injectIndicators(nameEl, lcName) {
        ...
        dot.className = 'bfc-dot'; dot.dataset.bfcFor = lcName; refreshDot(dot, info);
        const stateIcon = document.createElement('span');
        stateIcon.className = 'bfc-state-icon'; stateIcon.dataset.bfcStateFor = lcName; refreshStateIcon(stateIcon, info);
        nameEl.prepend(stateIcon); nameEl.prepend(dot);
    }
    ...
    /* ══════════════════════════════════════════
       MENTION DEDUP — fingerprint set, never cleared
    ══════════════════════════════════════════ */
    ...
    function refreshBar() {
        ...
        if (label) label.textContent = n === 1 ? 'You were mentioned — click to jump'
            : `Mentioned ${n}× · ${mentionIdx + 1}/${n}`;
    }
    ...
    function checkMention(msgEl) {
        ...
        if (new RegExp('@' + escRx(CFG.username) + '\\b', 'i').test(msgEl.textContent)) {
            ...
            if (CFG.mentionBrowser) browserNotify(msgEl.textContent.trim().slice(0, 80));
            }
        }
    }
    ...
    /* ══════════════════════════════════════════
       SCROLL HELPER
    ══════════════════════════════════════════ */
    function scrollIntoContainer(el, behavior = 'smooth') {
        if (!el) return;
        if (!msgListEl || !msgListEl.contains(el)) {
            el.scrollIntoView({ behavior, block: 'center' });
            return;
        }

        const containerTop = msgListEl.getBoundingClientRect().top;
        const elRect       = el.getBoundingClientRect();
        const elMid        = elRect.top - containerTop + (elRect.height / 2);
        const target       = msgListEl.scrollTop + elMid - (msgListEl.clientHeight / 2);
        msgListEl.scrollTo({ top: target, behavior });
    }
    ...
    function execSearch() {
        ...
        searchHits = [];
        processedMsgs.forEach(entry => {
            if (entry.bodyText.includes(term)) {
                searchHits.push(entry);

                if (entry.el.isConnected) entry.el.classList.add('bfc-s-match');
            }
        });
    ...
    function moveSearch(d) {
        ...
        const prev = searchHits[searchCursor];
        if (prev && prev.el.isConnected) prev.el.classList.remove('bfc-s-active');

        searchCursor = (searchCursor + d + searchHits.length) % searchHits.length;
        activateHit(searchCursor);
        updateSCount();
    }
    ...
    function activateHit(i) {
        ...
        if (hit.el.isConnected) {
            ...
            hit.el.classList.add('bfc-s-active');
            scrollIntoContainer(hit.el);
        } else if (msgListEl) {

            const ratio  = hit.idx / Math.max(1, processedMsgs.length - 1);
            const target = ratio * (msgListEl.scrollHeight - msgListEl.clientHeight);
            msgListEl.scrollTo({ top: target, behavior: 'smooth' });

            setTimeout(() => {
                searchHits.forEach(h => { if (h.el.isConnected) h.el.classList.add('bfc-s-match'); });
                if (hit.el.isConnected) {
                    hit.el.classList.add('bfc-s-active');
                    scrollIntoContainer(hit.el, 'auto');
                }
    ...
    function clearSearch(inp) {
        document.querySelectorAll('.bfc-s-match,.bfc-s-active').forEach(el =>
            el.classList.remove('bfc-s-match', 'bfc-s-active'));
    ...
    function updateSCount() {
        ...
        if (!el) return;
        el.textContent = searchHits.length
            ? `${searchCursor + 1}/${searchHits.length}`
            : (searchInput && searchInput.value ? '0' : '');
    }
    ...
    function updateMemberPopup() {
        ...
        memberPopup.innerHTML =
            `<div class="bfc-mp-hdr">
                <span class="bfc-col-on">● ${on} Online</span>
                <span class="bfc-col-idle">● ${idl} Idle</span>
                <span class="bfc-col-off">● ${off} Offline</span>
            </div>` +
            entries.map(m => {
                const icon = CFG.showTravelIcons ? (STATE_ICONS[m.state] || '') : '';
                const tip  = icon ? esc(m.stateDesc || capitalize(m.state)) : '';
                return `<div class="bfc-mp-item">
                    ...
                    ${icon ? `<span class="bfc-mp-state" title="${tip}">${icon}</span>` : ''}
    ...
    /* ══════════════════════════════════════════
       VISUAL INJECTION (idempotent)
    ══════════════════════════════════════════ */
    function injectVisuals(msgEl) {
        if (!msgEl.classList.contains('bfc-msg-container')) msgEl.classList.add('bfc-msg-container');
        if (!msgEl.querySelector('.bfc-cp-btn')) injectCopy(msgEl, msgEl);
        const nameEl = findNameEl(msgEl);
        if (nameEl && !nameEl.querySelector('.bfc-dot')) {
            injectIndicators(nameEl, nameEl.textContent.trim().replace(/[:\s]+$/, '').toLowerCase());
        }
        if (CFG.showTimestamps && !msgEl.querySelector('.bfc-ts')) injectTs(msgEl);
    }
    ...
    function processMsg(msgEl) {
        ...
        if (processedMsgFps.has(fp)) return;
        ...
        processedMsgs.push({ el: msgEl, bodyText, idx: processedMsgs.length });
    ...
    function browserNotify(text) {
        ...
        if (Notification.permission === 'granted')
            new Notification('💬 Torn Faction Chat', { body: text, icon: 'https://www.torn.com/favicon.ico' });
    ...
    function runScanner() {
        ...
        chatScanner = setInterval(() => {
            const box = document.querySelector('[id^="faction-"]');
    ...
    function buildSettingsPanel() {
        ...
        gearBtn.id = 'bfc-gear-btn'; gearBtn.innerHTML = '⚙️'; gearBtn.title = 'Better Faction Chat Settings';
        document.body.appendChild(gearBtn);

        const panel = document.createElement('div');
        panel.id = 'bfc-panel';
        panel.innerHTML = `
            <div id="bfc-ph"><h3>⚡ Better Faction Chat</h3><span id="bfc-pver">v${BFC_VERSION}</span></div>
            <div id="bfc-pb">
                ...
                <div class="bfc-row">
                    ...
                    <label class="bfc-sw"><input type="checkbox" id="s-snd"${CFG.soundEnabled?' checked':''}><span class="bfc-swt"></span></label>
                </div>
                <div class="bfc-vol-row">
                    <span class="lbl">Volume</span>
                    <input class="bfc-rng" type="range" id="s-vol" min="0" max="1" step="0.05" value="${CFG.volume}">
                    <span class="val" id="s-vol-lbl">${Math.round(CFG.volume*100)}%</span>
                </div>
                <div class="bfc-row">
                    <span>🖥️ Browser / OS notifications</span>
                    <label class="bfc-sw"><input type="checkbox" id="s-brw"${CFG.mentionBrowser?' checked':''}><span class="bfc-swt"></span></label>
                </div>
            ...
            <div id="bfc-credit">
                Developed by <a href="https://www.torn.com/profiles.php?XID=4141121" target="_blank" rel="noopener">sercann [4141121]</a>
            </div>
        `;
        ...
        const volLabel  = document.getElementById('s-vol-lbl');
        volSlider.addEventListener('input', () => { volLabel.textContent = Math.round(volSlider.value * 100) + '%'; });
        ...
        document.getElementById('bfc-test-btn').addEventListener('click', async () => {
            ...
            } catch(e) { setMsg(`✗ ${e.message}`, false); btn.textContent = '🔑 Test Key & Auto-detect Username'; }
        });
        ...
        document.getElementById('bfc-save-btn').addEventListener('click', () => {
            ...
            CFG.soundEnabled     = document.getElementById('s-snd').checked;
            CFG.volume           = parseFloat(document.getElementById('s-vol').value);
            CFG.mentionBrowser   = document.getElementById('s-brw').checked;
            ...
            saveCfg(CFG); startPoll(); chatBoxEl = null;
            setMsg('✓ Saved! Chat will refresh shortly.', true);
        });
